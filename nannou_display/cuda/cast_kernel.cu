...
void backCullPreprocessKernel(
    ...
    if (point_base < all_point_num) {              // for the last block, thread might be less than act_point_num
        ...
        const int next_pt_id = point_base + next_id * need_forward + 1 - need_forward;
        const int prev_pt_id = point_base + next_id * need_reverse - 1 + need_reverse;
        Point cur_pt = Point(points[point_base]);
        const Point cur_vec = cur_pt - Point(pose), prev_vec = cur_pt - Point(points[prev_pt_id]), next_vec = Point(points[next_pt_id]) - cur_pt;
...
void simpleDuplicateKernel(
    ...
    outputs[threadIdx.x << 1] = input_angle - 1e-4;
...
void pointIntersectKernel(
    ...
    __syncthreads();
    const int seg_base = (seg_boffset << 1) + blockIdx.x, ray_base = (ray_boffset << 1) + blockIdx.y;
    const int seg_id = seg_base * blockDim.y + threadIdx.y;
    const int ray_id = ray_base * blockDim.x + threadIdx.x;
    if (seg_id < all_seg_num && ray_id < all_ray_num && mesh_valid[seg_id] == true) {
        ...
        if ((in_range && not_singular) || (in_range_singular && !not_singular)) {
            const Point dir_vec(cosf(ray_angle), sinf(ray_angle)), obs_p(pose);
            Point p = Point(points[next_angle_id]) - Point(points[seg_id]);
            float swap_tmp = p.y;
            p.y = p.x;
            p.x = -swap_tmp;
